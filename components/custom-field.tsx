...
interface PopoverSelectFieldProps<TFieldValues extends FieldValues, TItem>
  extends UseControllerProps<TFieldValues> {
  className?: string;
  defaultLabel?: string;
...
interface PopoverCheckboxFieldProps<TFieldValues extends FieldValues, TItem>
  extends UseControllerProps<TFieldValues> {
  label: string;
  items: TItem[];
...
interface ToggleGroupFieldProps<TFieldValues extends FieldValues, TItem>
  extends UseControllerProps<TFieldValues> {
  label: string;
  items: TItem[];
  getItemKey: (item: TItem) => string | number;
...
interface RadioGroupFieldProps<TFieldValues extends FieldValues, TItem>
  extends UseControllerProps<TFieldValues> {
  label: string;
  items: TItem[];
  getItemKey: (item: TItem) => string | number;
...
export const InputField = <TFieldValues extends FieldValues>({
  ...
  return (
    <GenericField label={label} {...fieldProps} renderInput={renderInput} />
  );
};
...
const preventInvalidNumberInput = (
  ...
  if (
    (event.currentTarget.value === "" && ["0"].includes(event.key)) ||
