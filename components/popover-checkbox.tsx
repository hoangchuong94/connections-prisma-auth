...
export const PopoverCheckbox = <T,>({
    ...
    return (
        <div className={`flex flex-col space-y-2 ${disabled ? 'cursor-not-allowed opacity-50' : ''} ${className}`}>
            <Popover open={open} onOpenChange={setOpen}>
                <PopoverTrigger asChild>
                    <Button
                        variant="outline"
                        className="min-w-[200px] justify-between border dark:border dark:border-slate-400 dark:text-black"
                        disabled={disabled}
                    >
