...
import { updateCategory, deleteCategory, createCategory, getCategoryById } from '@/actions/category';
...
export default function CategoryModal() {
    ...
    const onSubmit = async (values: CategoryModalSchemaType) => {
        if (!values.name || values.genders.length === 0) return;
        try {
            setLoading(true);
            setErrorMessage('');
            setSuccessMessage('');
                    ...
                    {isFetching ? (
                        ...
                        <Form {...form}>
                            <form onSubmit={handleSubmit(onSubmit)} className="space-y-2">
                                <div className="rounded-md border border-gray-300 bg-slate-200 p-2 dark:text-black">
                                    <InputField
                                        name="name"
                                        label="Category Name"
                                        className="bg-white dark:border dark:border-slate-500"
